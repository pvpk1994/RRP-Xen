...
#define TRACE_IOTKIT_SECCTL_RESET_BACKEND_DSTATE() ( \
    trace_event_get_state_dynamic_by_id(TRACE_IOTKIT_SECCTL_RESET) || \
    false)

static inline void _nocheck__trace_iotkit_secctl_reset(void)
{
    if (trace_event_get_state(TRACE_IOTKIT_SECCTL_RESET) && qemu_loglevel_mask(LOG_TRACE)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log("%d@%zu.%06zu:iotkit_secctl_reset " "IoTKit SecCtl: reset" "\n",
                 getpid(),
                 (size_t)_now.tv_sec, (size_t)_now.tv_usec
                 );
    }
