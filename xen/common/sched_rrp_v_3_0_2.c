 ...
 **************** LAST UPDATED ON  September 16, 2021 *************************************************
 ...
 TODO: RRP-Xen multi-core CPUs (1 and 3) to accommodate DomUs with multiple VCPUS [Done]
 TODO: To test and build RRP-Xen multi-core to support CPUs (1 and 3) to accommodate DomUs with multiple VCPUs spanned across different CPUs [Remaining]
...
static void* aafsched_alloc_vdata(const struct scheduler *ops, struct vcpu *vc, void *dd)
{
	...
	if(avc == NULL)
	    return NULL;
	struct ps_pcpu_t *sched_pcpu = SCHED_PCPU(vc->processor);
	...
	/* Add schedule Entries VCPUs to per-CPU RUNQ schedule entries
         * RRP-Xen Multi core supported CPUs: 1 and 3
	 * Make sure only  non-idle VCPUs belonging to CPUs 1 || 3 are added to respective per-CPU RUNQs
	 */

/*
	 if(vc->domain->domain_id != 0 && (vc->processor == 1 || vc->processor == 3))
	 {
		// spin_lock_irqsave(&SCHED_PCPU(vc->processor)->pcpu_lock, flags_pcpu);
		printk("Function: %s  Domain: %X  VCPU: %d\n", __func__, vc->domain->handle, vc->vcpu_id);
		entry = sched_pcpu->num_schedule_entries;
		if (entry < PS_MAX_DOMAINS_PER_SCHEDULE)
		{
			memcpy(sched_pcpu->schedule[entry].dom_handle, vc->domain->handle, sizeof(vc->domain->handle));
			sched_pcpu->schedule[entry].vcpu_id = vc->vcpu_id;
			sched_pcpu->schedule[entry].wcet = DOM0_TS;
			sched_pcpu->schedule[entry].vc = vc;
...
/*
        else if(!is_idle_vcpu(vc) && (vc->processor == 1 || vc->processor == 3))
	{
	 ...
	 // Gain Per-CPU lock - Need to gain the lock
	 //  Release the global scheduler lock before releasing per-CPU lock
