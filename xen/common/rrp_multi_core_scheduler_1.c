 ...
 **************** LAST UPDATED ON MARCH 30, 2020 *************************************************
 ...
 TODO: Abrupt Crashing of DomU when trying to deliberately quit it using xl destroy [NOT URGENT]--->[WORKING]
 ...
 TODO: Scheduler failing for more than 1 domain per CPU since the other domain is unable 
       to find a valid vcpu left [VERY URGENT] ---> [FIXED]

 * Inclusion of per-cpu Locks to avoid list corruptions
 * This scheduler works fine, still failing at xl destroy stage
 *************************************************************************************************/
...
static void aaf_free_vdata(const struct scheduler *ops, void *priv)
{
  ...
  if(pvcpu == NULL)
	return;
 /* Make the deletion from the list only if its non-RRP-CPU */
 if(!is_idle_vcpu(pvcpu->vc)&& (pvcpu->vc->processor == 0 || pvcpu->vc->processor == 2))
 {
	list_del(&pvcpu->list_elem);
 }


 else if(!is_idle_vcpu(pvcpu->vc) && (pvcpu->vc->processor == 1 || pvcpu->vc->processor == 3))
 {
	cpu_id = pvcpu->vc->processor;
        struct ps_pcpu_t *pc = SCHED_PCPU(cpu_id);
	spin_lock(&pc->pcpu_lock); 
        list_del(&pvcpu->runq_elem);
	spin_unlock(&pc->pcpu_lock);
 }
 printk("[RRP-Xen 2.0] Destroyed VCPU: [Domain: %d. VCPU: %d]\n",pvcpu->vc->domain->domain_id, pvcpu->vc->vcpu_id);

 if(cpu_id == 1 || cpu_id == 3)
 {
   xfree(pvcpu);
   //spin_lock_irq(&SCHED_PRIV(ops)->lock);
   del_vcpu_pcpu(ops,cpu_id);
   //spin_unlock_irq(&SCHED_PRIV(ops)->lock);
   printk("Leaving aaf_free_vdata for CPU ID: %d\n", cpu_id);
   return;
 }
 else
{
